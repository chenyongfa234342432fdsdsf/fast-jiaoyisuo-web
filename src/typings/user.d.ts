...
export type MemberSafeVerifyWithdrawalPhoneSendReq = {
  type: number // 类型，1，手机号注册，2，极验验证，3，提现
  mobileCountryCode: string // 手机区号，未登陆必传
  mobile: string // 手机号，未登陆必传
  address: string // 提币地址
...
export type MemberSafeResetReq = {
  type_ind: string // 安全认证类型 1 手机 2 邮箱 3 google 认证码 4 密码
  google_secret_key: string // google 密钥
  google_secret_key_code: string // google 验证码
  email: string // 新认证的邮箱
  email_code: string // 新认证的邮箱验证码
  mobile_area_code: string // 新认证的手机号区号
  mobile_number: string // 新认证的手机号
  mobile_number_code: string // 新认证的手机号验证码
  contacts_number: string // 联系电话
  contacts_number_code: string // 联系电话验证码
  reset_photos_path: string // 认证照片 OSS 地址
  contacts_number_area: string // 联系电话区号
  uid: string // 会员 id
}
...
export interface ReturnVeritedType<T> {
  code: number
  msg: string
  pageNum: number
  time: number
...
export type MemberIdentityApplyReq = {
  birthday: string
...
export type MemberIdentityApplyRes = ReturnVeritedType<void>

export type MemberVerifyRecordReq = {
  identityType: number
}

export type MemberVerifyRecordRes = {
  isOk: boolean
  birthday: string
  cardBackPhotoUrl: string
  cardFrontPhotoUrl: string
...
export type MemberVerifyResultReq = {
  identityType: number
}

export type MemberVerifyResultRes = {
  identityStatus: number
  reason: string
}


export type MemberAdvancedApplyReq = {
  country: string
  city: string
  street: string
  addressProof: string
...
export type MemberAdvancedApplyRes = ReturnVeritedType<undefined>

export type MemberPermissionDetailReq = void

export type MemberPermissionDetailRes = {
  amount: string
  auditTime: number
  reason: string
  status: number
...
export type MemberPermissionDepositReq = void

export type MemberPermissionDepositRes = {
  depositAmount: number
  depositCurrency: string
}

export type MemberStatusPermissiontReq = void

export type MemberStatusPermissiontRes = ReturnVeritedType<void>

export type MemberPermissionUserInfoRes = {
  realName: string
}

export type MemberMerchantApplyReq = {
  address: string
  areacode: string
  email: string
  ...
  phone: string
  relation: string
  uploadIdcardUrl: string
  uploadVideoUrl: string
  weChart: string
}

export type MemberMerchantApplyRes = {}
...
export type MemberVerifyMemberByAccounResp = {
  ...
  mobileCountryCd?: string // 手机区号
  mobileNumber?: string // 手机号
  kycType: number // kyc 等级 1 为未认证 2 为个人标准认证 3 为个人高级认证 4 为企业认证
  email?: string // 邮箱
}
...
/** 刷新 token */
export type MemberAuthRefreshTokenReq = {
  refreshToken: string // 登陆成功后获取的 refreshToken 值
  tokenTtl: number
}

export type MemberAuthRefreshTokenResp = {
  accessToken: string // 新的 token
  refreshToken: string
  accessTokenExpireTime: number
  refreshTokenExpireTime: number
}

export type MemberBaseApplicationLanguageReq = {
  type: string
}